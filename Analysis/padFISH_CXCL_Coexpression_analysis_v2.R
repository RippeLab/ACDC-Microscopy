### This script quantifies the expression of target genes in the CXCL cluster to assess their co-expression patterns ###
# It uses the "quantNuclei.R" custom function to quantify the signal in a pre-selected subnuclear masks corresponding to the co-expression locus across target genes
...
#select nucmask file 
nucmask_sel <- allfilesmask[grep("_cellpose_masks.png$", allfilesmask)]
...
###### PART 2: FILTERING STEP FOR NUCLEAR MASKS  ######
...
# inspect filtered image
display(nucmasks)  #If nuclei at borders are still included, re-assign pre-filtered nucmasks and change border_size (e.g., 15-20)

#OPTIONAL: check number of cells in the final mask
Ncell_initial <- length(unique(as.vector(nucmasks_filt2)))
print(Ncell_initial)

###### PART 3: QUANTIFICATION OF SIGNAL IN CO-LOCALIZED MASKs WITHIN NUCLEAR MASKS ######
...
inttable <- intensity_table %>% group_by(Ch) %>%      #first create on the initial file a new column with the row numbers
  mutate(row_number=row_number())
inttable2 <- subset(inttable, select = c(row_number,Mean,Ch))  #then subset the table to only have the row number, mean and channel number
all_ch_inttable <- inttable2 %>%                 #then create a wide table, where each channel gets its own column
...
final_table[, "DAPI"] <- NULL   #exclude DAPI from the following analysis
 
colnames(intensity_table) <- c("locus", "area", colnames(intensity_table)[3:ncol(intensity_table)])   #add size info per co-expression locus
final_table <- merge(final_table, intensity_table[, 1:2], by="locus")
final_table$intensity <- final_table[, grep("CXCL", colnames(final_table))] %>% rowSums()   #add overall intensity per locus
#save intensity table in work directory 
write.csv(final_table, "/output/filepath/_final_table_intensity.csv", row.names = FALSE)

###### PART 4: DATA INSPECTION and FILTERING ######
...
#Save plots as interim data in output folder (OPTIONAL)
...
###### PART 5: BINARIZE DATA based on INTENSITY THRESHOLDING ######
...
CXCL1_cutoff <- 500    #insert value (Values used - Rep1: 800, Rep2: 400 Rep3: 500)
...
CXCL2_cutoff <- 800   #insert value (Values used - Rep1: 1000, Rep2: 1000 Rep3: 800)
...
CXCL3_cutoff <- 300    #insert value (Values used - Rep1: 700, Rep2: 600 Rep3: 300)
...
CXCL8_cutoff <- 1000    #insert value (Values used - Rep1: 1000, Rep2: 1000 Rep3: 1000)
...
###### PART 6: CREATE FINAL TABLE for subsequent analysis ######
...
# create average between the three runs from the percentage value and obtain then standard deviation
# final file that is generated (for 30 and 240 min condition) will be used in the follow-up analysis on co-expression between padFISH and sequencing.
...
# Use "_stats_avg_sd_perc.csv" files for 30 and 240 min condition in the follow-up script for co-expression: "_20251003_Seurat_CXCL_CoExpression_v4.html"
# Final CSV files are provided as Source Data for Fig. 2d. Microscopy images are collected in the BioImage archive repository.
